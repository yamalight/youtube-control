        ...
        <div class="flex items-center">
          ...
          <div class="flex flex-col p-4">
            <h2 class="font-bold text-lg text-tial-400">
              <a href="#" onClick=${() => addChannel(channel)}>${channel.name}</a>
            </h2>
          </div>
        </div>
      </div>
...
const Modal = ({close, channels}) => {
  const store = useStore();

  return html`
    <div
      className="main-modal fixed w-full h-100 inset-0 z-50 overflow-hidden flex justify-center items-center animated fadeIn faster modal-bg"
    >
      <div
        className="border border-teal-500 shadow-lg modal-container bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg z-50 overflow-hidden max-h-full"
      >
        <div className="modal-content py-4 text-left px-6 flex flex-col max-h-screen">
          ...
          <div className="my-5 overflow-auto">
            ${channels
              .filter(ch => !store.channels.find(storeCh => storeCh.name === ch.name))
              .map(
                channel =>
                  html`
...
export default ({channels}) => {
  ...
  return html`
    ...
    ${showModal
      ? html`
          <${Modal} close=${() => setModal(false)} channels=${channels} />
        `
      : ''}
  `;
};
